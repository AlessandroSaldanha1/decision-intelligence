...
const MOCK_ANALYSIS = {
  riskScore: 65,
  summary: 'A demanda é viável tecnicamente mas requer clareza em alguns pontos de escopo antes de iniciar. Os principais riscos estão relacionados a integrações externas e volume de dados.',
  ambiguities: [
  ...
  risks: [
    { description: 'Complexidade de integração com APIs externas', severity: 'high', mitigation: 'Criar camada de abstração e circuit breaker' },
    { description: 'Performance com grande volume de dados históricos', severity: 'medium', mitigation: 'Implementar paginação e cache' },
    { description: 'Falta de especificação de edge cases de autenticação', severity: 'low', mitigation: 'Revisar fluxo com time de produto' },
  ],
  dependencies: [
...
export default async function AnalysisPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const riskColor = MOCK_ANALYSIS.riskScore >= 70 ? 'text-red-400' : MOCK_ANALYSIS.riskScore >= 40 ? 'text-yellow-400' : 'text-green-400'

  return (
    <div className="mx-auto max-w-3xl px-4 py-12 space-y-8">
      <div className="space-y-1">
        <div className="text-xs text-gray-500 uppercase tracking-wide font-medium">Claude Analysis</div>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-gray-400">Ambiguidades ({MOCK_ANALYSIS.ambiguities.length})</h2>
        <div className="space-y-2">
          {MOCK_ANALYSIS.ambiguities.map((a, i) => (
            <div key={i} className="flex gap-3 rounded-lg border border-yellow-500/20 bg-yellow-600/10 p-3">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {MOCK_ANALYSIS.risks.map((r, i) => (
            <div key={i} className={`rounded-lg border p-4 space-y-1 ${SEVERITY_COLORS[r.severity]}`}>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">{r.description}</span>
                <span className={`text-xs font-semibold uppercase rounded-full px-2 py-0.5 ${SEVERITY_COLORS[r.severity]}`}>{r.severity}</span>
              </div>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-gray-400">Perguntas em Aberto</h2>
        <div className="space-y-2">
      ...
      <div className="flex gap-3">
        <Link href={`/demand/${id}/artifacts`} className="flex-1 rounded-lg bg-violet-600 px-4 py-2.5 text-center text-sm font-semibold text-white hover:bg-violet-500 transition-colors">
          Gerar Artefatos
        </Link>
      </div>
    </div>
